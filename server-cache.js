function makeCache(){
	...
	var configTemplate = JSON.parse(JSON.stringify(config));
	configTemplate.cacheOn = false;
	
	var index = indexTemplate(configTemplate);
	var my_scripts = [];
	var css_links = [];
			...
			if(script !== undefined) {
				if(('src' in script)){
					if(script.src.indexOf('/assets') == 0){
						...
						if(file == 'assets/js/handlebars.js'){
							file = 'assets/js/handlebars.runtime.js'
						}
	...
	for(var i = 0; i < files.length; i++){
		...
		if(files[i][0] != '.'){
			...
			templateJs += "\ntemplateCache['/" + file + "']=Handlebars.template(" + Handlebars.precompile(fileContents).toString() + ");";
		}
	}
...
module.exports.makeCache = makeCache;